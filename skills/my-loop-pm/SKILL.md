...
- 全程凡是要用户决定或向用户交付，都按 `my-loop-eli-beginner` 的口径说话：先说对他意味着什么，项目术语就地换成他懂的话。
...
交付完成时：确认所有 tickets 已完成、必需验收已通过，`phase` 置 `done`；然后按 `my-loop-eli-beginner` 收口——一段讲给初学者的人话：做成了什么、动了哪些文件、做了哪些决策（含 `ponytail:` 天花板）、怎么验的、还剩什么。这是交付的一部分，不是独立阶段。初始化或单纯设置工具不冒充业务任务交付。
