---
name: my-loop-pm
description: my-loop v3 总入口。项目根 CLAUDE.md 写了「本项目用 my-loop v3」的，收到功能开发、加接口、改数据结构、重构、改现有行为、bug 修复一类请求，一律先加载这个——它管分级、对齐、派活、对账、什么时候找人。用户要在项目里初始化/安装 my-loop v3 时也加载。改一行文案、改常量这种谁都不影响的小事不要加载，直接改完。
---
